	...
	if (sPage.search("rdRequestForwarding=Form") == -1) {
        ...
        for (var i = eleInputs.length - 1; i > -1; i--) {
            ...
            if (!eleKeeped.has(eleInput.name)) {
                ...
                try {
                        if (eleInput.id == "rdHiddenRequestForwarding") {//we are currently looking on rdHiddenRequestForwarding (it has more priority). And we do have the other one with same name. So, replacing with rdHiddenRequestForwarding
                            var savedElem = eleKeeped.get(eleInput.name);
                            eleRemoved.push(savedElem.parentNode.removeChild(savedElem));
                            eleKeeped.set(eleInput.name, eleInput)
                        }
                    } catch (e) { }
                }
        ...
        for (var k = eleTextAreas.length - 1; k > -1; k--) {
            if (eleKeeped.has(eleTextAreas[k].name)) {
                var eleInput = eleKeeped.get(eleTextAreas[k].name);
